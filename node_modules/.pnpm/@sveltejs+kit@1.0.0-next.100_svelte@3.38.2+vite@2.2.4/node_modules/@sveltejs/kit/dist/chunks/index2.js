...
import { p as posixify, m as mkdirp } from './utils.js';
...
function create_manifest_data({ config, output, cwd = process.cwd() }) {
	...
	function find_layout(file_name, dir) {
		...
		return files.find((file) => fs__default.existsSync(path.resolve(cwd, file)));
	}
	...
	return {
		assets: fs__default.existsSync(assets_dir) ? list_files(assets_dir, '') : [],
		layout,
		error,
		components,
		routes
